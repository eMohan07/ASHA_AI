    ...
    <div className="flex flex-col h-full bg-slate-950/50">
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-slate-500 space-y-4">
            ...
            <p>I am CareLink Pro Assistant. How can I help?</p>
          </div>
        ) : (
          messages.map(m => (
      ...
      <div className="p-4 bg-slate-900 border-t border-slate-800">
        <form onSubmit={handleSubmit} className="flex relative items-center">
          <input
            className="w-full bg-slate-950 border border-slate-700 rounded-full py-4 pl-6 pr-14 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-slate-500 shadow-inner"
            value={input}
          ...
          <button 
            ...
            type="submit" 
            className="absolute right-2 p-2 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 disabled:text-slate-600 rounded-full text-white transition-colors"
          >
